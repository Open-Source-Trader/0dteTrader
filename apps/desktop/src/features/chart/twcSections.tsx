...
export interface TwcSettingsSection {
  id: string;
  label: string;
  content: ReactNode;
}
 ...
 *  fields), rather than one flat concatenated list — the desktop settings
 *  tree needs to link to and render a single group at a time; the phone
 *  sheet (TwcSettingsBody) just concatenates all of them in order. */
export function buildTwcSections(
  ...
  return [
    ...
    section(
      ...
      toggleRow('Show Zones', 'showPremiumDiscountZones'),
    ),
    section(
      'confluence',
