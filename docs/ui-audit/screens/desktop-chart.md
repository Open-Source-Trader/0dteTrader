...
- **Exact fix:**
  ...
  chart.subscribeCrosshairMove((param) => {
    ...
    setLegend(
      bar
        ? `O ${Format.price(bar.open)}  H ${Format.price(bar.high)}  L ${Format.price(bar.low)}  C ${Format.price(bar.close)}`
        : null,
    );
  });
  ```
...
- **Exact fix:**
  ...
  .menu-dropdown {
    animation: menu-in 150ms cubic-bezier(0.32, 0.72, 0, 1);
  }
  ```
