...
- **Exact fix:** Append to `base.css`:
  ...
  @media (prefers-reduced-motion: reduce) {
    :root { --toast-anim: 1ms linear; --sheet-anim: 1ms linear; }
  }
  ```
...
- **Exact fix:** Move to CSS classes. In ToastView.tsx:
  ...
  .toast-success { border-color: color-mix(in srgb, var(--pnl-positive) 60%, transparent); }
  .toast-success .toast-icon { color: var(--pnl-positive); }
  .toast-error { border-color: color-mix(in srgb, var(--pnl-negative) 60%, transparent); }
  .toast-error .toast-icon { color: var(--pnl-negative); }
  .toast-info { border-color: color-mix(in srgb, var(--app-accent) 60%, transparent); }
  .toast-info .toast-icon { color: var(--app-accent); }
  ```
