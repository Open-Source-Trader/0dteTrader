...
- **What/Why:** States. In `confirmArmedOrder()` the catch blocks set `previewError` (TradeViewModel.swift:256,258), so a *submission* failure ("Market closed", "Insufficient buying power") appears in the card styled as if the preview failed — directly under "Est. buying power" — and the Retry button next to it calls `loadPreview()`, not a resubmit. Wrong semantics, wrong recovery action.
