...
- **Score justifications:**
  - Composition 5 — centered pill with on-grid 16pt outer margins is sound, but the capsule is placed at y=4 from the *physical* screen top (safe-area violation, collides with status bar/Dynamic Island) and inner paddings 14/10 break the 8pt grid.
  ...
  - Color 4 — text on capsule is 14.1:1 (excellent), but the *only* style signal is a 1pt stroke at 0.6 opacity: error red = 2.26:1 and info accent = 2.53:1, both failing WCAG 3:1 for UI components; icons are untinted (`.primary`).
